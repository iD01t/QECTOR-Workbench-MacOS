...
(compiled Rust core + public Python layer, **v1.0.0**). Features **17 decoders**,
**10 code families** (including qLDPC and color codes), batch/streaming decode,
hardware detection, a resilient self/auto-debug backend, a **82-tool MCP server**,
...
```
QectorWorkbench-Portable.exe --mcp     # 82-tool stdio MCP server (no display needed)
```
...
### Decoder Lab
Test any of **17 decoders** with configurable error rate and seed. View error,
...
- a **five-figure publication suite** at 300 dpi: Tanner graph, parity-check
  sparsity pattern, mean decode latency per decoder, observed logical failure
  fraction against the physical error rate, and a speed-against-accuracy plot
  with the Pareto front marked;
- **Methods** and **Data Availability** sections stating the seeding scheme
  (`seed = base + i`), what the latency figure excludes, and that the failure
  fraction at 25 trials is a screening estimate rather than a converged logical
  error rate;
- **deposit metadata** from the Lab & Personal Info tab (author, ORCID,
  affiliation, DOI, funding, keywords) under a CC-BY-4.0 licence line, with a
  formatted citation.

Two house rules apply to everything generated: no typographic dashes, and no
invented attribution. With no profile saved a report says "Unattributed" rather
than inventing a plausible author name.

### Lab & Personal Info
Holds the deposit profile above, and installs a decoder licence key where the
decoder actually reads it (`~/.qector/license.key`), reporting the resulting
tier, for example `Community (max distance 7; GPU disabled; GNN disabled)`.
GPU batch decoding is Enterprise-tier gated: a machine can have a working CUDA
device and still be refused at Community tier, and this readout is where that
shows.

### MCP Server
**82-tool** Model Context Protocol server (stdio JSON-RPC 2.0) for programmatic
