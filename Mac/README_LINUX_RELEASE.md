...
This Linux package is fully self-contained: it bundles its own Python 3.11
runtime, Tcl/Tk, the scientific stack, **and** the `qector_decoder_v3` wheel. On
first launch the bundled decoder wheel is activated into a per-user managed site
automatically. **No system Python, pip, or internet connection is required** and
no online update checks are performed — the app runs entirely from what ships in
the box.
...
Two packages are provided — the same app with dependency tuning for each distro
family. Pick yours and install from the folder containing the `.deb` files:

```bash
# Ubuntu / Debian / Mint (resolves dependencies automatically)
sudo apt install ./qector-workbench_1.0.0_amd64_ubuntu.deb

# antiX / MX Linux (dpkg, then pull any missing deps)
sudo dpkg -i ./qector-workbench_1.0.0_amd64_antix.deb && sudo apt-get -f install
```

The packages are built on a glibc 2.31 baseline, so they run on **Ubuntu 20.04+,
Debian 11+, Linux Mint 20+, antiX 21+ / MX 21+, Fedora 32+, openSUSE Leap 15.3+**
and newer. A desktop menu entry (Science / Education) and the official icon are
installed automatically.

## Using the application

Launch the app from the desktop menu or the terminal:

```
qector-workbench                      # launch the GUI
