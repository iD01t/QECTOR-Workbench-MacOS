...
async def test_all_mcp_functions():
    ...
    bench_res = await call_mcp_tool("run_benchmark", {"code_family": "repetition", "n_samples": 30, "seed": 7})
    ...
    test_params = {
        ...
        "set_config": {"config": {"theme_mode": "dark"}},
    }

    results = {}

    for tool in tools:
        params = test_params.get(tool, {}).copy()
    ...
    print("OVERALL STATUS:", "ALL MCP FUNCTIONS OK" if overall else "SOME TOOLS NEED ATTENTION")
    print("=" * 85)
