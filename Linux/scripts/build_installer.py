...
import version as _version  # noqa: E402  (needs REPO on sys.path first)

#: Always the single source of truth. Hardcoding it here has shipped a release
#: labelled 3.5.1 while version.py said otherwise.
...
def write_installer_version() -> Path:
    """Emit the ISPP include that carries the version into installer.iss."""
    inc = REPO / "installer_version.iss"
    inc.write_text(
        "; Generated by scripts/build_installer.py from version.py.\n"
        "; Do not edit by hand; edits are overwritten on every build.\n"
        f'#define MyAppVersion "{VERSION}"\n',
        encoding="utf-8",
    )
    print(f"Wrote {inc}  (MyAppVersion {VERSION})")
    return inc
