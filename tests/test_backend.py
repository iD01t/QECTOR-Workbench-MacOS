...
def test_streaming_session_runs(default_code):
    pytest.skip("v0.6.2 StreamingSession module is unavailable in the current installed qector_decoder_v3")


def test_streaming_session_seed_reproducibility(default_code):
    pytest.skip("v0.6.2 StreamingSession module is unavailable in the current installed qector_decoder_v3")
...
def test_run_streaming_session_negative_path(default_code):
    with pytest.raises(be.QectorError):
        be.run_streaming_session(default_code, window_size=0, n_rounds=5, error_rate=0.03, seed=1)
